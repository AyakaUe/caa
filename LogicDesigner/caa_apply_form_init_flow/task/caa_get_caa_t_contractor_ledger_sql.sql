...
SELECT
    ...
    , ctcl.receive_company_cd                   -- 受入会社コード
    , ctcl.receive_company_name                 -- 受入会社名
    , ctcl.receive_company_short                -- 受入会社略称
    , ctcl.receive_dept_cd                      -- 受入部署コード
    , COALESCE(ctcl.receive_dept_name, id.department_name) AS receive_dept_name -- 受入部署名
    , ctcl.receive_user_cd                      -- 受入担当者：ユーザコード
    , ctcl.receive_employee_no                  -- 受入担当者：社員番号
    , ctcl.receive_user_name                    -- 受入担当者：氏名
    , ctcl.receive_post_cd                      -- 受入担当者：役職コード
    , ctcl.receive_post_name                    -- 受入担当者：役職名
    , ctcl.matter_no                            -- 案件番号
    , ctcl.apply_date                           -- 起票日
    ...
    , ctcl.doc_management_no                    -- 稟議番号（文書番号）
...
FROM
    ...
    LEFT OUTER JOIN ( -- 受入部署名を組織マスタから取得
        SELECT
            department_cd
            , department_name 
        FROM
